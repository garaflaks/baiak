...
function onStepIn(cid, item, position, fromPosition)
	...
	doItemSetAttribute(doCreateItem(1437, 1, {x = 1180, y = 444, z = 6, stackpos = 1}), "uid", 2499)
			...
			for _, pid in ipairs(getPlayersOnline()) do
				if getPlayerStorageValue(pid, 71) == 1 or getPlayerStorageValue(pid, 72) == 1 then
					doPlayerSendTextMessage(pid, MESSAGE_STATUS_WARNING, "[CTF EVENT] O " .. getCreatureName(cid) .. " trouxe a bandeira vermelha pra base verde e ganhou 1 ponto!\n Verdes: "..getGlobalStorageValue(8).." X "..getGlobalStorageValue(9).." Vermelhos.")
				end
			end
	...
	doItemSetAttribute(doCreateItem(1435, 1, {x = 1234, y = 444, z = 6}), "uid", 2500)	
			...
			for _, pid in ipairs(getPlayersOnline()) do
				if getPlayerStorageValue(pid, 71) == 1 or getPlayerStorageValue(pid, 72) == 1 then
					doPlayerSendTextMessage(pid, MESSAGE_STATUS_WARNING, "[CTF EVENT] O " .. getCreatureName(cid) .. " trouxe a bandeira verde pra base vermelha e ganhou 1 ponto!\n Verdes: "..getGlobalStorageValue(8).." X "..getGlobalStorageValue(9).." Vermelhos.")
				end
			end
