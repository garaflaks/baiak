...
function onStepIn(cid, item, position, fromPosition)
	...
	if getGlobalStorageValue(battlefield.storage) > 0 then
		if getGlobalStorageValue(battlefield.storage) % 2 == 0 then
			...
			doTeleportThing(cid, battlefield.pos_team_1)   
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Você se juntou ao time " .. battlefield.team1Name .. "!")       
		else
			...
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Você se juntou ao time " .. battlefield.team2Name .. "!")
		end
		...
		if getGlobalStorageValue(battlefield.storage) == 0 then
			...
			addEvent(doBroadcastMessage, 60*1000, "[BATTLEFIELD] o evento vai começar AGORA!")
			addEvent(OpenWallBattle, 60*1000)
		else
			doBroadcastMessage("[BATTLEFIELD] nos estamos esperando "..getGlobalStorageValue(battlefield.storage).." para o evento começar.")
		end
	end
	return true
end