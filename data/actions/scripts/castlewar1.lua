      ...
      if sto_ativ == 1 or sto_ativ == -1 then
         ...
         guild_sto = getGlobalStorageValue(storages[3])
            ...
            if guild ~= guild_sto then            
               ...
               doBroadcastMessage("[Castle24h] O(a) "..getCreatureName(cid).." acabou de conquistar o castelo "..castle_one_name.." para a guild \""..guildname.."\" pela "..sto_vezes_total.."ª vez(s). Premio: 10% a mais de exp.", 22)
			   doCreatureSay(cid, "[Castle24h] Sua guild ganhou 10% a mais de experiencia, relogue para ativar!", TALKTYPE_ORANGE_1)
                 ...
                 if premio_por_vezes == 1 then
                    if getPlayerStorageValue(cid,storages[1]) == premio_vezes then
                       if getItemWeightById(premio,1) <= getPlayerFreeCap(cid) then
                          doBroadcastMessage("[Castle24h] O "..getCreatureName(cid).." ganhou uma(o) "..getItemNameById(premio).." como recompensa das suas "..premio_vezes.." conquistas!", 21)
                          doPlayerAddItem(cid, premio,premio_cont)
                          doPlayerSendTextMessage(cid, 18, "[Castle24h] Voce recebeu o(a) "..getItemNameById(premio).." como recompensa das suas "..premio_vezes.." conquistas!")
                        -----------------------------------------------  --  
                       ...
                       doPlayerSendTextMessage(cid, 18, "[Castle24h] Voce nao tem capacidade para ganhar o item. Vamos verificar novamente em 10 segundos!")
                       addEvent(additem,10000,cid,premio,premio_cont,premio_reset)
                       end
                    end                                        
                 end               
            else
              doPlayerSendCancel(cid,"[Castle24h] Este castelo ja foi conquistado pela sua guild!")
              doSendMagicEffect(position, 2)
            end
      else
      doPlayerSendCancel(cid,"[Castle24h] O sistema está desativado")
      end      
   else
   doPlayerSendCancel(cid,"[Castle24h] É nescessario ter guild para dominar o castelo!")
   doSendMagicEffect(position, 2)
   end
end
...
function additem(cid,premio,premio_cont,premio_reset)
 ...
 if getItemWeightById(premio,1) <= getPlayerFreeCap(cid) then
  ...
  doPlayerSendTextMessage(cid, 18, "[Castle24h] Voce nao tem capacidade para ganhar o item. Verificando novamente em 10 segundos!")
