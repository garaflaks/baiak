...
setGlobalStorageValue(11, 1)	
		...
		for _, pid in ipairs(getPlayersOnline()) do
			if getPlayerStorageValue(pid, 71) == 1 or getPlayerStorageValue(pid, 72) == 1 then
				doPlayerSendTextMessage(pid, MESSAGE_STATUS_WARNING, "[CTF EVENT] O " .. getCreatureName(cid) .. " roubou a bandeira do time verde!")
			end
		end
	...
	elseif item.uid == 2500 and getPlayerStorageValue(cid, 71) == 1  then
		...
		for _, pid in ipairs(getPlayersOnline()) do
			if getPlayerStorageValue(pid, 71) == 1 or getPlayerStorageValue(pid, 72) == 1 then
				doPlayerSendTextMessage(pid, MESSAGE_STATUS_WARNING, "[CTF EVENT] O " .. getCreatureName(cid) .. " roubou a bandeira do time vermelho!")
			end
		end
	end
	return true
...
function voltabandeira (cid)
	if getPlayerStorageValue(cid, 72) == 1 and getPlayerStorageValue(cid, 82) == os.time() then
		...
		for _, pid in ipairs(getPlayersOnline()) do
			if getPlayerStorageValue(pid, 71) == 1 or getPlayerStorageValue(pid, 72) == 1 then
				doPlayerSendTextMessage(pid, MESSAGE_STATUS_WARNING, "[CTF EVENT] O " .. getCreatureName(cid) .. " ficou muito tempo com a bandeira verde e ela foi devolvida para sua base.")
			end
		end
	elseif getPlayerStorageValue(cid, 71) == 1 and getPlayerStorageValue(cid, 83) == os.time() then	
		...
		for _, pid in ipairs(getPlayersOnline()) do
			if getPlayerStorageValue(pid, 71) == 1 or getPlayerStorageValue(pid, 72) == 1 then
				doPlayerSendTextMessage(pid, MESSAGE_STATUS_WARNING, "[CTF EVENT] O " .. getCreatureName(cid) .. " ficou muito tempo com a bandeira vermelha e ela foi devolvida para sua base.")
			end
		end
	end
return true
