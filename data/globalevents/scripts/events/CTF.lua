function onTime ()
	doItemSetAttribute(doCreateItem(1387, 1, {x = 142, y = 41, z = 7, stackpos = 1}), "aid", 1432)
	doBroadcastMessage("[CTF EVENT] o evento vai começar em 4 minutos! O teleporte vai ser fechado quando o evento começar.", MESSAGE_STATUS_WARNING)
	...
	addEvent(startCtf, 5 * 60 * 1000)
	return true
end

function startCtf()
	doRemoveItem(getTileItemById({x = 142, y = 41, z = 7}, 1387).uid, 1)
	if getGlobalStorageValue(6) > 4 and getGlobalStorageValue(7) > 0 then
		doBroadcastMessage("[CTF EVENT] boa sorte a todos os jogadores! O teleporte foi fechado.", MESSAGE_STATUS_WARNING)		
		...
		doBroadcastMessage("[CTF EVENT] o evento não iniciou pois nao possui player suficiente.")
		for _, cid in ipairs(getPlayersOnline()) do
			if getPlayerStorageValue(cid, 71) == 1 or getPlayerStorageValue(cid, 72) == 1 then
...
function endCtf()
	...
	if getGlobalStorageValue(9) > getGlobalStorageValue(8) then
		doBroadcastMessage("[CTF EVENT] encerrado! Time vermelho foi o vencedor! "..getGlobalStorageValue(9).." X "..getGlobalStorageValue(8)..". Recompensa: 35kk de ExP e 10 Event Coins.")
		for _, cid in ipairs(getPlayersOnline()) do
	...
	elseif getGlobalStorageValue(8) > getGlobalStorageValue(9) then
		doBroadcastMessage("[CTF EVENT] encerrado! Time verde foi o vencedor! "..getGlobalStorageValue(8).." X "..getGlobalStorageValue(9)..". Recompensa: 35kk de ExP e 10 Event Coins.")
		for _, cid in ipairs(getPlayersOnline()) do
	...
	elseif getGlobalStorageValue(8) == getGlobalStorageValue(9) then
		doBroadcastMessage("[CTF EVENT] encerrado! Empate de "..getGlobalStorageValue(8).." X "..getGlobalStorageValue(9).."! Todos participantes irão ganhar uma parte da recompensa: 20kk de ExP e 5 Event Coins.")
		for _, cid in ipairs(getPlayersOnline()) do
			if getPlayerStorageValue(cid, 71) == 1 or getPlayerStorageValue(cid, 72) == 1 then
