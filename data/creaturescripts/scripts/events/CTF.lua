...
function onPrepareDeath(cid, deathList, lastHitKiller, mostDamageKiller)
	if getPlayerStorageValue(cid, 82) >= os.time() then
		...
		for _, pid in ipairs(getPlayersOnline()) do
			if getPlayerStorageValue(pid, 71) == 1 or getPlayerStorageValue(pid, 72) == 1 then
				doPlayerSendTextMessage(pid, MESSAGE_STATUS_WARNING, "[CTF EVENT] O " .. getCreatureName(cid) .. " morreu com a bandeira verde e ela foi devolvida para sua base.")
			end
		end
	elseif getPlayerStorageValue(cid, 83) >= os.time() then
		...
		for _, pid in ipairs(getPlayersOnline()) do
			if getPlayerStorageValue(pid, 71) == 1 or getPlayerStorageValue(pid, 72) == 1 then
				doPlayerSendTextMessage(pid, MESSAGE_STATUS_WARNING, "[CTF EVENT] O " .. getCreatureName(cid) .. " morreu com a bandeira vermelha e ela foi devolvida para sua base.")
			end
		end
	end
	return true
