...
function addeventAccount(cid, count)  --function by Mirto, MiltonHit - otserv.com.br
            db.executeQuery("UPDATE `accounts` SET `event_points` = event_points + '"..count.."' WHERE `name` ='"..getPlayerAccount(cid).."'")
end

function geteventBalance(cid)  --function by Mirto, MiltonHit - otserv.com.br
         local skpo = db.getResult("SELECT * FROM `accounts` where `name`='"..getPlayerAccount(cid).."'")
         return skpo:getDataInt("event_points")
end
