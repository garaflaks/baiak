...
ChatChannel* Chat::createChannel(Player* player, uint16_t channelId/*, bool isBroadcasting*/)
{
	...
	switch(channelId)
	{
		...
		case CHANNEL_PRIVATE:
		{
			...
			if(/*(!player->isPremium() && !isBroadcasting) || */getPrivateChannel(player))
				return NULL;
			...
			for(uint16_t i = 100; i < 10000; ++i)
			{
				...
				if((newChannel = new PrivateChatChannel(i, "Cast Channel", flags)))
				{
...
ChannelsList Chat::getChannelList(Player* player/*, bool isBroadcasting*/)
{
	ChannelsList list;
	if(!player || player->isRemoved())
		return list;
	...
	if(!hasPrivate/* && (player->isPremium() || isBroadcasting)*/)
