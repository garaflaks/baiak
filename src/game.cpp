	...
	const Position& movingCreaturePos, const Position& toPos, bool delay)
{
    ...
    {
        player->sendTextMessage(MSG_STATUS_SMALL, "Você não pode empurrar players em area protection zone.");
        return false;
    }
...
bool Game::playerCreatePrivateChannel(uint32_t playerId/*, bool isBroadcasting*/)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved()/* || (!player->isPremium() && !isBroadcasting)*/)
		return false;

	ChatChannel* channel = g_chat.createChannel(player, 0xFFFF/*, isBroadcasting*/);
...
bool Game::playerRequestChannels(uint32_t playerId/*, bool isBroadcasting*/)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

	player->sendChannelsDialog(g_chat.getChannelList(player/*, isBroadcasting*/));
...
std::string _text = asLowerCaseString(text);
    ...
    for(StringVec::iterator it = strVector.begin(); it != strVector.end(); ++it)
    {
        ...
        for(ii=0; ii < int(length); ii++)
        {
            ...
            {
                player->sendTextMessage(MSG_STATUS_SMALL, "Você não pode enviar este tipo de mensagem no nosso servidor.");
                return false;
                break;
            }
        }
	...
	const std::string& text, uint32_t statementId)
{
 ...
 if(player->getLevel() < 300) { // 300 level min to send pm.
    player->sendTextMessage(MSG_STATUS_SMALL, "Você não possui level suficiente para enviar mensagem privada.");
    return false;
}
